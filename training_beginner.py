        ...
        "steps": [
            ...
            },
            {
                ...
                "body": (
                    ...
                    "</div>"
                    "<p style=\"font-size:12px;margin:6px 0;line-height:1.45\">"
                    "<b>Both <code>with</code> blocks on this slide are context managers.</b> "
                    "<code>with lock:</code> releases the lock in <code>__exit__</code>, so an "
                    "exception inside the block cannot leave it held — that is how you avoid a "
                    "deadlock. Leaving <code>with ThreadPoolExecutor(...)</code> calls "
                    "<code>shutdown(wait=True)</code>, which is exactly why the measured thread "
                    "count above drops back to 1 after the block. Full protocol and use cases "
                    "on <b>Slide 26 — Context Manager</b>."
                    "</p>"
                    "<p class=\"step-result\">"
