...
REAL_LIFE: dict[int, str] = {
    ...
    17: (
        ...
        "<p><b>1) Generator — one row per <code>yield</code></b><br>"
        "Read the file inside <code>with</code>; hand back one split row at a time.</p>"
        '<div class="step-pre">'
        "# INPUT\n"
        "def csv_reader(path):\n"
        "    with open(path, encoding=\"utf-8\") as f:\n"
        ...
        "\n"
        "# OUTPUT\n"
        "row → ['id', 'name', 'amount']  (one row — not whole file)"
        "</div>"
        "<p><b>2) Streaming export — write as you read</b><br>"
        "Transform each row and write immediately; memory stays flat on 50M rows.</p>"
        '<div class="step-pre">'
        "# INPUT\n"
        "def export(path, out):\n"
        ...
        "\n"
        "# OUTPUT\n"
        "out file grows line by line — no MemoryError"
        "</div>"
        "<p><b>3) Bad approach — load entire file</b><br>"
        "<code>read().split()</code> loads everything into RAM at once.</p>"
        '<div class="step-pre">'
        "# INPUT\n"
