...
<tr><td>pass</td><td><b>pass = this block is intentionally empty for now.</b> A stub — define structure today, add real code later. Does nothing at runtime but satisfies syntax.<br><br><b>Use when:</b> stub <code>def</code>, empty <code>class</code>, custom <code>Exception</code>.<br><br><b>Later:</b> remove <code>pass</code> and write your logic.<br><br><b>Does NOT:</b> skip a loop — use <code>continue</code>.<br><br><b>Bug:</b> bare <code>except: pass</code> swallows Ctrl+C and all errors — catch a specific type instead. __CSHARP_PASS_BTN__</td><td><code>def todo(): pass</code></td></tr>
...
<table class="data-tbl term-tbl">
<tr><th>Term</th><th>Meaning</th><th>Quick example</th></tr>
<tr><td>@property</td><td>Built-in <b>data</b> descriptor — managed attribute (start here). __CSHARP_PROPERTY_BTN__</td><td><code>@property def price:</code></td></tr>
<tr><td>Getter</td><td>Method called when attribute is read.</td><td><code>def price(self): return self._price</code></td></tr>
<tr><td>Setter</td><td>Method called when attribute is assigned — validate here.</td><td><code>@price.setter</code></td></tr>
<tr><td>Managed attribute</td><td>Looks like a field, runs code on get/set.</td><td><code>p.price = 25</code></td></tr>
<tr><td>Descriptor</td><td>Helper object (security guard) behind the scenes — <code>__get__</code> / <code>__set__</code> / <code>__delete__</code>.</td><td><code>price = PositiveDecimal()</code></td></tr>
<tr><td>Data descriptor</td><td><code>__get__</code> + <code>__set__</code> — every write goes through the helper (same job as <code>@property</code>).</td><td>can validate assignments</td></tr>
